...
//            mockValidation.Setup(x => x.CleanText(It.IsAny<string>(), It.IsAny<bool>(), It.IsAny<List<string>>(), It.IsAny<HashSet<char>>())).Returns("abc");
            ...
            var redirectResult = result as RedirectToActionResult;
            object val;
            redirectResult.RouteValues.TryGetValue("errorMessage", out val);
            val.ToString().Should().Be("Enter the name of a service, its address, postcode or a combination of these");
        }
